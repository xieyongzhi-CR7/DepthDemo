...
Shader "Unlit/JingShen01"
{
    ...
    CGINCLUDE
        ...
        v2f_blur vert_blur(appdata_img v)
        {
            ...
            o.uv = v.texcoord.xy;
            o.uv01 = uv + _Offsets.xyxy * float4(1,1,-1,-1);
            o.uv23 = uv + _Offsets.xyxy * float4(1,1,-1,-1) * 2.0;
            o.uv45 = uv + _Offsets.xyxy * float4(1,1,-1,-1) * 3.0;
            return o;
        }

        fixed4 frag_blur(v2f_blur i):SV_TARGET
        {
            float4 colr = fixed4(0,0,0,0);
            colr += 0.4 * tex2D(_MainTex,i.uv);
            colr += 0.15 * tex2D(_MainTex,i.uv01.xy);
            colr += 0.15 * tex2D(_MainTex,i.uv01.zw);
            colr += 0.10 * tex2D(_MainTex,i.uv23.xy);
            colr += 0.10 * tex2D(_MainTex,i.uv23.zw);
            colr += 0.05 * tex2D(_MainTex,i.uv45.xy);
            colr += 0.05 * tex2D(_MainTex,i.uv45.zw);
            return colr;
